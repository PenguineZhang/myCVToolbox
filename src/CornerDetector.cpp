...
namespace pzCV
{
    ...
    vector<pair<int, int>> CornerDetector::harrisDetector(cv::Mat& img, cv::Mat& kernel, string filter)
    {
        cv::Mat Ix, Iy;
        ...
        if(filter == "box")
        {
            filter_kernel = cv::Mat(cv::Size(3,3), CV_32F, cv::Scalar(1)) / 9;
        }
        else if (filter == "gaussian")
        {
            // generate 2d gaussian kernel with mean of 0 and variance of 1.0 in both dimension
            gaussianKernelGenerator(filter_kernel, 3, 3, 0, 0, 1.0, 1.0);
        }
        else
        {
            cout << "Specified filter is not available, defaulting to box filter" << endl;
            filter_kernel = cv::Mat(cv::Size(3,3), CV_32F, cv::Scalar(1)) / 9;
        }
        ...
        for(int i = row_start; i < row_end; i++)
        {
            for(int j = col_start; j < col_end; j++)
            {
                ...
                A.at<float>(0, 0) = cv::sum(I_x2(cv::Range(i - 1, i + 2), cv::Range(j - 1, j + 2)).mul(filter_kernel))[0];//.mul(filter))[0];
                A.at<float>(1, 1) = cv::sum(I_y2(cv::Range(i - 1, i + 2), cv::Range(j - 1, j + 2)).mul(filter_kernel))[0];// / kernel_element_size;
                A.at<float>(1, 0) = cv::sum(I_xy(cv::Range(i - 1, i + 2), cv::Range(j - 1, j + 2)).mul(filter_kernel))[0];// / kernel_element_size;
